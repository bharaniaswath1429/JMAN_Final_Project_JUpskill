...
const AdminTraining = () => {
  ...
  const handleQuizSubmit = (e) => {
    ...
    const apiUrl = newQuiz.id
      ? `http://localhost:8000/api/courses/${newQuiz.id}`
      : "http://localhost:8000/api/courses";
  
    const requestMethod = newQuiz.id ? axios.put : axios.post;
  
    requestMethod(apiUrl, newQuiz, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
      .then((response) => {
        if (newQuiz.id) {
          const updatedCourses = courses.map((course) =>
            course.id === newQuiz.id ? response.data : course
          );
      ...
      .catch((error) => {
        if (
          ...
          error.response.data.message === "Course already exists!"
        ) {
          ...
          {filteredCourses.length === 0 ? (
            ...
            filteredCourses.map((course) => (
              <tr key={course.id}>
                ...
                <td>{course.questions?.length || 0}</td>
                <td>
                  <FaEdit
                    style={{
                      cursor: "pointer",
